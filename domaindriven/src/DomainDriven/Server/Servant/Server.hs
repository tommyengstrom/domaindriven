...
newtype CbQueryServer (model :: Type) m a = CbQuery ((forall n. MonadIO n => n model) -> m a)
...
instance
  ...
  where
  type ServerT (Cmd model event (Verb method status ctypes a)) m = CmdServer model event m a
  hoistServerWithContext _ _ f (Cmd action) = Cmd $ \model -> f (action model)

  route _ context delayedServer = case getContextEntry context :: Persistence (LookupPersistance context) of
    Persistence p ->
...
instance
  ...
  where
  type ServerT (CbQuery model (Verb method status ctypes a)) m = CbQueryServer model m a
...
instance
  ...
  where
  type ServerT (CbCmd model event (Verb method status ctypes a)) m = CbCmdServer model event m a

  hoistServerWithContext _ _ f (CbCmd action) = CbCmd $ \transact -> f (action transact)
