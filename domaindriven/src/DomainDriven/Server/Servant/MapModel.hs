...
class MapModelAndEvent serverFrom serverTo model model' event event' where
  mapModelAndEvent :: (model -> model') -> (event' -> event) -> serverFrom -> serverTo
...
instance MapModelAndEvent (CbCmdServer model' event' m a) (CbCmdServer model event m a) model model' event event' where
  mapModelAndEvent proj inj (CbCmd server) =
    CbCmd $ \transact -> server (\action -> transact $ \model -> ((. proj) *** map inj) <$> action (proj model))

instance Functor m => MapModelAndEvent (CmdServer model' event' m a) (CmdServer model event m a) model model' event event' where
  mapModelAndEvent proj inj (Cmd server) =
    Cmd $ \model -> ((. proj) *** map inj) <$> server (proj model)

instance MapModelAndEvent (QueryServer model' m a) (QueryServer model m a) model model' event event' where
  mapModelAndEvent proj _ (Query server) = Query $ server . proj

instance MapModelAndEvent (CbQueryServer model' m a) (CbQueryServer model m a) model model' event event' where
  mapModelAndEvent proj _ (CbQuery server) = CbQuery $ \model -> server (proj <$> model)

instance MapModelAndEvent server' server model model' event event' => MapModelAndEvent (a -> server') (a -> server) model model' event event' where
  mapModelAndEvent proj inj server a = mapModelAndEvent proj inj (server a)

instance (MapModelAndEvent server1' server1 model model' event event', MapModelAndEvent server2' server2 model model' event event') => MapModelAndEvent (server1' :<|> server2') (server1 :<|> server2) model model' event event' where
  mapModelAndEvent proj inj (server :<|> server2) = mapModelAndEvent proj inj server :<|> mapModelAndEvent proj inj server2

instance
  (MapModelAndEvent t u model model' event event', MapModelAndEvent (NP I ts) (NP I us) model model' event event') =>
  MapModelAndEvent (NP I (t ': ts)) (NP I (u ': us)) model model' event event'
  where
  mapModelAndEvent proj inj (I t :* ts) = I (mapModelAndEvent proj inj t) :* mapModelAndEvent proj inj ts

instance MapModelAndEvent (NP I fields) (NP I fields') model model' event event' => MapModelAndEvent (SOP I '[fields]) (SOP I '[fields']) model model' event event' where
...
instance MapModel (CbCmdServer model' event m a) (CbCmdServer model event m a) model model' where
  mapModel proj (CbCmd server) =
    CbCmd $ \transact -> server (\action -> transact $ \model -> first (. proj) <$> action (proj model))

instance Functor m => MapModel (CmdServer model' event m a) (CmdServer model event m a) model model' where
...
instance MapModel server' server model model' => MapModel (a -> server') (a -> server) model model' where
  mapModel proj server a = mapModel proj (server a)

instance (MapModel server1' server1 model model', MapModel server2' server2 model model') => MapModel (server1' :<|> server2') (server1 :<|> server2) model model' where
...
instance MapModel (NP I fields) (NP I fields') model model' => MapModel (SOP I '[fields]) (SOP I '[fields']) model model' where
...
instance MapEvent (CbCmdServer model event' m a) (CbCmdServer model event m a) event event' where
  mapEvent inj (CbCmd server) =
    CbCmd $ \transact -> server (\action -> transact $ fmap (second (map inj)) . action)

instance Functor m => MapEvent (CmdServer model event' m a) (CmdServer model event m a) event event' where
...
instance MapEvent server' server event event' => MapEvent (a -> server') (a -> server) event event' where
  mapEvent inj server a = mapEvent inj (server a)

instance {-# OVERLAPS #-}  MapEvent server server event event' where
  mapEvent _ = id

instance (MapEvent server1' server1 event event', MapEvent server2' server2 event event') => MapEvent (server1' :<|> server2') (server1 :<|> server2) event event' where
...
instance MapEvent (NP I fields) (NP I fields') event event' => MapEvent (SOP I '[fields]) (SOP I '[fields']) event event' where
